...
export default class UserService {
  ...
  async getUserDataWithoutPasswordById(id: string) {
    ...
    if (!user)
      throw new CustomHttpException(MSG.USER_NOT_FOUND, HttpStatus.NOT_FOUND);
  ...
  //       { status_code: 404, error: 'User not found' },
  ...
  // async exportUserDataAsJsonOrExcelFile(
  //   format: FileFormat,
  //   userId: string,
  //   res: Response,
  // ) {
  ...
  //     res.setHeader(
  //       'Content-Disposition',
  //       `attachment; filename="${userId}-data.xlsx"`,
  //     );
  ...
  //     res.setHeader(
  //       'Content-Disposition',
  //       `attachment; filename="${userId}-data.json"`,
  //     );
